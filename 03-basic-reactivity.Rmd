...
These names are already taken by `base::range()` and `stats::var()`. Using them for naming reactive expressions makes Shiny confused. In this case code will because when defining `range()` Shiny takes `var()` as a function instead of a reactive object.
